...
- **南極**：正距円筒（作者確認）。
- **世界**：出自は**メルカトル**（作者）。ただし**ピクセル予算に収める縦圧縮でメルカトルの高緯度引き伸ばしがほぼ相殺**され、実測では正距円筒の方がGISとよく合う：

  | 世界バンドの投影 | 陸海不一致(before) | warp後 |
  |---|---|---|
  | equirect（既定）| **12.7%** | **7.2%** |
  | mercator | 13.3% | 7.9% |

  よって既定は equirect。出自どおりメルカトルにしたい場合は `bands.csv` の world 行の proj を `mercator` に変更（緯度は latN≈78, latS≈-68 が最良）。
