...
- マップは **5120×2560**。ただし **単一の正距円筒ではない**。**世界map＋南極map を縦に合体し、指定ピクセル数に収めるため縦に押しつぶした**もので、**バンドごとに縦のスケール（縦横比）が異なる**（＝「2種類の縦横比」）。継ぎ目は**下から367.5px（正規化0.8562）**。
  - 世界バンド：行 0–2192（正規化 0–0.8562）↔ 緯度 **+90°〜−50°**（出自メルカトル／実用上は正距円筒が最良）
  - 南極バンド：行 2192–2560（0.8562–1.0）↔ 緯度 **−54°〜−90°**（正距円筒）
- このピースワイズ投影（`bands.csv`）を前提に実地理(GIS)と比較すると、陸海の食い違いは **約12.0%**（単一正距円筒という誤前提だと16.8%に見えていた）。
...
各バンドは `equirect` / `mercator` を選べる（`bands.csv` の6列目）。

- **南極**：正距円筒（作者確認）。
- **世界**：出自は**メルカトル**（作者）。ただし**最上行を北極(+90°)に合わせて押しつぶした**ため、純メルカトルでは有限の端に極を置けず、実用上は正距円筒が最良。継ぎ目を正確値（下から367.5px）にした上での実測：

  | 世界バンドの投影 | 陸海不一致(before) |
  |---|---|
  | equirect（既定）| **12.03%** |
  | mercator(latN≈78) | 12.45% |

  縦方向だけの相関ではメルカトルが上回るが、北極を端に固定する制約のため全体一致は equirect が良い。出自どおりメルカトルにしたい場合は `bands.csv` の world 行を `mercator`（latN≈78, seam≈−62）に。

> **継ぎ目の確定値**：世界map（メルカトル）と南極map（正距円筒）の境界は**下から367–368px**（作者）＝上から行2192.5＝正規化 **0.8562**。
