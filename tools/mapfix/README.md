...
- **poly は世界バンドと南極バンドを別々にフィット**（既定 `--per-band`。世界=`--poly-degree 5`、南極=`--ant-degree 3`）。両バンドは投影が異なるため、1枚の大域多項式だと継ぎ目をまたいで2つの無関係な歪みを平均してしまう。バンド独立化で南極も改善し、潰れる小プロヴィンスも減る（squeezed 56→36）。`--no-per-band` で旧大域。
